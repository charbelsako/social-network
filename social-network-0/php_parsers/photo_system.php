...
if (isset($_FILES["avatar"]["name"]) && $_FILES["avatar"]["tmp_name"] != ""){
	...
	//add the profile pic into an album called profile pics
	// WedFeb272120452013RAND.jpg
	$db_pic_name = date("DMjGisY")."".rand(1000,9999).".".$fileExt; // name of image that goes into the album profile pictures	
	$newName = copy("../user/$log_username/$db_file_name", "../user/$log_username/$db_pic_name");
    
	$sql = "INSERT INTO photos(user,gallery,filename,uploaddate) VALUES('$log_username','Profile Pictures','$db_pic_name', NOW() )";
	$query = mysqli_query($db_conx, $sql);
	if($query){
...
if (isset($_FILES["photo"]["name"]) && isset($_POST["gallery"])){
	...
	$gallery = preg_replace('#[^a-z 0-9,]#i', '', $_POST["gallery"]);
	$caption = preg_replace('#[^a-z 0-9,]#i', '', $_POST["caption"]);
	...
	$sql = "INSERT INTO photos(user, gallery, filename, description, uploaddate) VALUES ('$log_username','$gallery','$db_file_name', '$caption', now())";
	$query = mysqli_query($db_conx, $sql);
	mysqli_close($db_conx);
